...
pub fn main() !void {
    ...
    const exe_name = if (argv.len > 0) std.fs.path.basename(argv[0]) else "focus_fuse_zig";
    ...
    core.pipeline.run(allocator, &cfg) catch |err| switch (err) {
        error.MismatchedImageSizes => {
            try std.fs.File.stderr().writeAll("ERROR: all fused inputs must have the same dimensions.\n");
            exitWithReport(1);
        },
        error.MismatchedImageFormats => {
            try std.fs.File.stderr().writeAll("ERROR: all fused inputs must have the same pixel format.\n");
            exitWithReport(1);
        },
